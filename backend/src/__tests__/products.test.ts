...
beforeAll(() => {
  ...
  seedProducts(testDb);
  svc = new ProductService(testDb);
  app = createApp(testDb); // HTTP integration tests share same in-memory DB
});
  ...
  describe("list()", () => {
    ...
    it("searches by partial name (case-insensitive)", () => {
      ...
      result.products.forEach((p) =>
        expect(p.name.toLowerCase()).toContain("sneaker")
      );
    });
...
describe("Product HTTP API", () => {
  ...
  it("GET /api/products?search=... filters results", async () => {
    const res = await request(app).get("/api/products?search=sneaker");
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  });
  ...
  it("DELETE /api/products/:id deletes a product", async () => {
    ...
    const del = await request(app).delete(`/api/products/${id}`).set("Authorization", `Bearer ${adminToken}`);
  ...
  it("GET /api/products/:id/stock returns stock count", async () => {
    ...
    const id = created.body.data.id;
    const res = await request(app).get(`/api/products/${id}/stock`);
    expect(res.status).toBe(200);
...
describe("Cart logic (stock enforcement)", () => {
  ...
  function updateQuantity(
    ...
    if (qty < 0) return { cart, error: "Quantity cannot be negative" };
    if (qty > stock)
      return { cart, error: `Only ${stock} unit(s) available` };
    if (qty === 0) return { cart: removeFromCart(cart, productId) };
    return {
      cart: cart.map((item) =>
  ...
  it("removes item when quantity set to 0", () => {
    const { cart } = addToCart([], mockProduct);
    const { cart: updated } = updateQuantity(
      cart,
      mockProduct.id,
      0,
      mockProduct.stock
